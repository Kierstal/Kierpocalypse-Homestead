-- Kierpocalyptic Homestead - Kierpocalyptic Rising preset
--
-- Cloned from vanilla Rising with Rising's zombie settings preserved, then
-- tuned to Kierstal's preferences:
--   - Zombie respawn OFF (RespawnMultiplier 0.1 -> 0). Zombies still spawn at
--     start and migrate, but a cleared area stays cleared.
--   - KH_Compat options set to her defaults (all defer-to-mod ON, taming ON,
--     spawn-override ON, start-with-companion OFF). Picking this preset on the
--     New Game screen auto-applies these without needing Custom Sandbox.
-- Zombie suppression for debug is still handled via an in-game toggle
-- ("Kill Zombies (KH)" - right-click on self), not via this preset.


return {
    Version = 6,
    Zombies = 5,
    Distribution = 1,
    ZombieVoronoiNoise = true,
    ZombieRespawn = 3,
    ZombieMigrate = true,
    DayLength = 4,
    StartYear = 1,
    StartMonth = 7,
    StartDay = 9,
    StartTime = 2,
    DayNightCycle = 1,
    ClimateCycle = 1,
    FogCycle = 1,
    WaterShut = 3,
    ElecShut = 3,
    AlarmDecay = 2,
    WaterShutModifier = 14,
    ElecShutModifier = 14,
    AlarmDecayModifier = 14,
    -- Loot density tuned for BWO VOriginal's "pre-apocalypse Week One"
    -- framing: people are still alive, homes have pantries and closets full
    -- of their stuff. Bumped 2026-05-30. Values mirror KH_LootBoost.lua so
    -- both new-game (via preset) and runtime override land the same place.
    FoodLootNew = 2.5,
    LiteratureLootNew = 1.5,
    SkillBookLoot = 0.8,
    RecipeResourceLoot = 1.0,
    MedicalLootNew = 1.5,
    SurvivalGearsLootNew = 2.0,
    CannedFoodLootNew = 2.5,
    WeaponLootNew = 0.4,
    RangedWeaponLootNew = 0.4,
    AmmoLootNew = 0.4,
    MechanicsLootNew = 1.5,
    OtherLootNew = 2.0,
    ClothingLootNew = 2.5,
    ContainerLootNew = 2.5,
    KeyLootNew = 1.0,
    MediaLootNew = 2.0,
    MementoLootNew = 2.0,
    CookwareLootNew = 2.5,
    MaterialLootNew = 3.0,
    FarmingLootNew = 2.5,
    ToolLootNew = 2.5,
    RollsMultiplier = 2.0,
    RemoveStoryLoot = false,
    RemoveZombieLoot = false,
    ZombiePopLootEffect = 0,
    InsaneLootFactor = 0.05,
    ExtremeLootFactor = 0.2,
    RareLootFactor = 0.7,
    NormalLootFactor = 2.0,
    CommonLootFactor = 3.5,
    AbundantLootFactor = 5.0,
    Temperature = 3,
    Rain = 3,
    ErosionSpeed = 4,
    ErosionDays = 0,
    Farming = 3,
    CompostTime = 2,
    StatsDecrease = 3,
    NatureAbundance = 3,
    Alarm = 2,
    LockedHouses = 5,
    StarterKit = true,
    Nutrition = true,
    FoodRotSpeed = 3,
    FridgeFactor = 3,
    SeenHoursPreventLootRespawn = 0,
    HoursForLootRespawn = 0,
    MaxItemsForLootRespawn = 5,
    ConstructionPreventsLootRespawn = true,
    WorldItemRemovalList = "Base.Hat, Base.Glasses, Base.Maggots, Base.Slug, Base.Slug2, Base.Snail, Base.Worm, Base.Dung_Mouse, Base.Dung_Rat",
    HoursForWorldItemRemoval = 24.0,
    ItemRemovalListBlacklistToggle = false,
    TimeSinceApo = 1,
    PlantResilience = 3,
    PlantAbundance = 3,
    EndRegen = 3,
    Helicopter = 1,
    MetaEvent = 2,
    SleepingEvent = 1,
    GeneratorFuelConsumption = 0.1,
    GeneratorSpawning = 4,
    AnnotatedMapChance = 4,
    CharacterFreePoints = 0,
    ConstructionBonusPoints = 4,
    NightDarkness = 3,
    NightLength = 3,
    BoneFracture = false,
    InjurySeverity = 1,
    HoursForCorpseRemoval = 216.0,
    DecayingCorpseHealthImpact = 3,
    ZombieHealthImpact = false,
    BloodLevel = 3,
    ClothingDegradation = 3,
    FireSpread = true,
    DaysForRottenFoodRemoval = -1,
    AllowExteriorGenerator = true,
    MaxFogIntensity = 1,
    MaxRainFxIntensity = 1,
    EnableSnowOnGround = true,
    AttackBlockMovements = true,
    SurvivorHouseChance = 3,
    VehicleStoryChance = 3,
    ZoneStoryChance = 3,
    AllClothesUnlocked = false,
    EnableTaintedWaterText = true,
    EnableVehicles = true,
    CarSpawnRate = 4,
    ZombieAttractionMultiplier = 1.0,
    VehicleEasyUse = false,
    InitialGas = 3,
    FuelStationGasInfinite = false,
    FuelStationGasMin = 0.7,
    FuelStationGasMax = 1.0,
    FuelStationGasEmptyChance = 10,
    LockedCar = 3,
    CarGasConsumption = 0.6,
    CarGeneralCondition = 3,
    CarDamageOnImpact = 3,
    DamageToPlayerFromHitByACar = 1,
    TrafficJam = true,
    CarAlarm = 3,
    PlayerDamageFromCrash = true,
    SirenShutoffHours = 0.0,
    ChanceHasGas = 2,
    RecentlySurvivorVehicles = 3,
    MultiHitZombies = true,
    RearVulnerability = 2,
    SirenEffectsZombies = true,
    AnimalStatsModifier = 4,
    AnimalMetaStatsModifier = 5,
    AnimalPregnancyTime = 3,
    AnimalAgeModifier = 4,
    AnimalMilkIncModifier = 4,
    AnimalWoolIncModifier = 4,
    AnimalRanchChance = 6,
    AnimalGrassRegrowTime = 240,
    AnimalMetaPredator = false,
    AnimalMatingSeason = true,
    AnimalEggHatch = 4,
    AnimalSoundAttractZombies = false,
    AnimalTrackChance = 6,
    AnimalPathChance = 6,
    MaximumRatIndex = 25,
    DaysUntilMaximumRatIndex = 90,
    MetaKnowledge = 3,
    SeeNotLearntRecipe = true,
    MaximumLootedBuildingRooms = 50,
    EnablePoisoning = 1,
    MaggotSpawn = 1,
    LightBulbLifespan = 3.0,
    FishAbundance = 3,
    LevelForMediaXPCutoff = 11,
    LevelForDismantleXPCutoff = 0,
    BloodSplatLifespanDays = 0,
    LiteratureCooldown = 90,
    NegativeTraitsPenalty = 1,
    MinutesPerPage = 2.0,
    KillInsideCrops = true,
    PlantGrowingSeasons = true,
    PlaceDirtAboveground = false,
    FarmingSpeedNew = 1.0,
    FarmingAmountNew = 1.0,
    MaximumLooted = 0,
    DaysUntilMaximumLooted = 90,
    RuralLooted = 0.5,
    MaximumDiminishedLoot = 20,
    DaysUntilMaximumDiminishedLoot = 3650,
    MuscleStrainFactor = 0.7,
    DiscomfortFactor = 0.8,
    WoundInfectionFactor = 1.0,
    NoBlackClothes = true,
    EasyClimbing = true,
    MaximumFireFuelHours = 8,
    FirearmUseDamageChance = 2,
    FirearmNoiseMultiplier = 1.0,
    FirearmJamMultiplier = 1.0,
    FirearmMoodleMultiplier = 1.0,
    FirearmWeatherMultiplier = 1.0,
    FirearmHeadGearEffect = true,
    ClayLakeChance = 0.05,
    ClayRiverChance = 0.05,
    GeneratorTileRange = 20,
    GeneratorVerticalPowerRange = 3,
    Basement = {
        SpawnFrequency = 4,
    },
    Map = {
        AllowMiniMap = true,
        AllowWorldMap = true,
        MapAllKnown = false,
        MapNeedsLight = true,
    },
    ZombieLore = {
        Speed = 3,
        SprinterPercentage = 0,
        Strength = 3,
        Toughness = 3,
        Transmission = 1,
        Mortality = 5,
        Reanimate = 3,           -- never (suppress reanimation just in case)
        Cognition = 3,
        DoorOpeningPercentage = 0,
        CrawlUnderVehicle = 5,
        Memory = 3,
        Sight = 2,
        Hearing = 3,
        SpottedLogic = true,
        ThumpNoChasing = false,
        ThumpOnConstruction = true,
        ActiveOnly = 1,
        TriggerHouseAlarm = false,
        ZombiesDragDown = true,
        ZombiesCrawlersDragDown = false,
        ZombiesFenceLunge = false,
        ZombiesArmorFactor = 1.0,
        ZombiesMaxDefense = 60,
        ChanceOfAttachedWeapon = 6,
        ZombiesFallDamage = 1.0,
        DisableFakeDead = 3,
        PlayerSpawnZombieRemoval = 1,
        FenceThumpersRequired = 25,
        FenceDamageMultiplier = 1.0,
    },
    ZombieConfig = {
        PopulationMultiplier = 0.15,
        PopulationStartMultiplier = 1.0,
        PopulationPeakMultiplier = 1.0,
        PopulationPeakDay = 28,
        RespawnHours = 72.0,
        RespawnUnseenHours = 16.0,
        RespawnMultiplier = 0.0,  -- KH: respawn OFF; a cleared area stays cleared.
        RedistributeHours = 18.0,
        FollowSoundDistance = 100,
        RallyGroupSize = 20,
        RallyGroupSizeVariance = 50,
        RallyTravelDistance = 30,
        RallyGroupSeparation = 15,
        RallyGroupRadius = 4,
        ZombiesCountBeforeDelete = 300,
    },
    MultiplierConfig = {
        Global = 1.0,
        GlobalToggle = true,
        Fitness = 1.0,
        Strength = 1.0,
        Sprinting = 1.0,
        Lightfoot = 1.0,
        Nimble = 1.0,
        Sneak = 1.0,
        Axe = 1.0,
        Blunt = 1.0,
        SmallBlunt = 1.0,
        LongBlade = 1.0,
        SmallBlade = 1.0,
        Spear = 1.0,
        Maintenance = 1.0,
        Woodwork = 1.0,
        Cooking = 1.0,
        Farming = 1.0,
        Doctor = 1.0,
        Electricity = 1.0,
        MetalWelding = 1.0,
        Mechanics = 1.0,
        Tailoring = 1.0,
        Aiming = 1.0,
        Reloading = 1.0,
        Fishing = 1.0,
        Trapping = 1.0,
        PlantScavenging = 1.0,
        FlintKnapping = 1.0,
        Masonry = 1.0,
        Pottery = 1.0,
        Carving = 1.0,
        Husbandry = 1.0,
        Tracking = 1.0,
        Blacksmith = 1.0,
        Butchering = 1.0,
        Glassmaking = 1.0
    },
    -- KH_Compat options. Mirrors the per-toggle defaults from sandbox-options.txt
    -- but applied at preset-pick time so Custom Sandbox isn't required.
    KHCompat = {
        -- Defer to compat mods (Kierstal's compatibility-first stance)
        DeferToLifestyle_Hygiene = true,
        DeferToLifestyle_Toilet  = true,
        DeferToSaph_Recipes      = true,
        DeferToSaph_BeefJerky    = true,
        DeferToHGTS_Climate      = true,
        DeferToFool_Containers   = true,
        -- Pets sibling-expansion on AE
        UsePetsOnAE              = true,
        DeferToMeowboid_Cats     = true,
        -- Bandits/BWO taming on (low spawn, no apocalyptic events)
        TameBandits              = true,
        TameBanditsWeekOne       = true,
        -- Spawn override ON so ParkRanger lands at the ranger station coord
        EnableSpawnLocations     = true,
        -- Start alone, find people in the world
        StartWithCompanion       = false,
    },
}
